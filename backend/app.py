...
def approve_activity(activity_id):
    """
    Approve a pending activity and execute it (send email if it's an email activity)
    ...
    try:
        ...
        # If this is an email activity, send the email now
        from services.email_sender import send_approved_email
        send_result = send_approved_email(activity_id)
        
        if send_result["status"] == "success":
            return jsonify({
                "status": "success",
                "message": f"Activity {activity_id} approved and email sent",
                "email_sent": send_result.get("sent", False),
                "timestamp": send_result.get("timestamp")
            }), 200
        else:
            # Email send failed, but activity is still approved
            return jsonify({
                "status": "partial_success",
                "message": f"Activity approved but email send failed: {send_result.get('error')}",
                "email_sent": False
            }), 200
